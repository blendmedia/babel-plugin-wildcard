    ...
    visitor: {
      ImportDeclaration(path, state) {
        ...
        if (isExplicitWildcard) {
          ...
          filenameRegex = filenameGlob.replace(
            /[*\.\(\[\)\]]/g,
            (character) => {
              switch (character) {
                case "*":
        ...
        for (var i = node.specifiers.length - 1; i >= 0; i--) {
          ...
          if (
            ...
            !_fs.existsSync(dir + ".js") &&
            !_fs.existsSync(dir + "/index.js")
          ) {
            addWildcard = true;
        ...
        if (addWildcard) {
          ...
          try {
            ...
            console.warn(
              `Wildcard for ${name} points at ${src} which is not a directory.`
            );
            return;
          }
